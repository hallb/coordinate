...
|----|-------------|----------|---------------------|
| FR-001 | Record a new expense with: service date, Person (the family member who incurred the expense), expense category, amount, and provider name. **Progressive disclosure:** the Person may be selected from existing Household members *or* created inline during the same expense-entry flow (name and date of birth) without navigating away to Settings first. The workflow must not require that all reference data (e.g. every Household member) exist before starting an expense. | Must | Expense is stored and visible in the claim dashboard. All five fields are required before the expense is considered complete. Inline Person creation produces the same Person aggregate as Settings (FR-040); no duplicate Person records. |
...
| FR-046 | Handle pre-recorded external submission. When an expense has been partially or fully processed through a plan outside this Household (e.g., a Person's coverage in another Household), the user can record the outcome (amount paid, EOB) as a pre-recorded submission. The Routing Engine picks up from the remainder through this Household's plans. Cross-household information flows via user-initiated document sharing (EOBs, receipts), not plan data sharing. See GLO-035. | Should | User can record an external plan's payment details and attach the EOB; system routes the remainder correctly through this Household's plans. |

### Routing Engine

| ID | Requirement | Priority | Acceptance Criteria |
|----|-------------|----------|---------------------|
| FR-010 | For a given expense, determine the next applicable plan using the following ordered rules: (1) employee-first rule (own plan before spouse's plan), (2) birthday rule for dependents (primary is the parent whose birthday is earliest in the calendar year), (3) duration of coverage as a tiebreaker. The Routing Engine must account for External Coverage (GLO-035) in COB ordering: when an external plan has higher COB priority than internal plans, the engine recommends "submit to [External Plan] first" without evaluating eligibility or limits. | Must | Given a test family configuration and a dependent expense, the system returns the correct primary plan in accordance with CLHIA Guideline G4. Test cases must include: (a) standard birthday rule, (b) parents with the same calendar birthday (falls back to duration-of-coverage tiebreaker), (c) employee-first override. When External Coverage is configured and has higher priority, the system recommends processing externally first. |
...
| FR-015 | Surface the routing decision to the user with a plain-language explanation. ("Submit to [Plan Name] because [reason]", e.g., "Submit to Sobia's Health Plan because Ben's plan has reached its annual maximum for Paramedical services.") When the next step is External Coverage, explain accordingly (e.g., "Submit to Mira's spouse's plan (external) first because it is primary per the birthday rule."). | Must | Explanation is shown before the user confirms submission. External Coverage recommendations are clearly labeled as external. |
...
| FR-047 | Maintain an **insurer directory** (insurer reference records: at minimum display name and optional portal URL). When adding or editing a Coverage, the user selects an existing insurer from the directory or creates a new directory entry once; the same insurer must not need to be re-keyed for each separate Coverage (e.g. health, dental, HCSA from the same carrier). Each Coverage continues to store an insurer reference (or snapshot) suitable for routing and history; the directory is the authoritative place to create and reuse insurer identity. | Must | User can create insurers in the directory and pick one when configuring a Coverage. Adding a second Coverage for the same real-world insurer reuses the directory entry (pre-filled or selected), not full duplicate manual entry of name and URL. |
...
| FR-045 | Record External Coverage for a Person within a Household. When a Person has coverage under a plan managed outside this Household, the user can record a lightweight reference: person covered, insurer name, plan type, and COB priority position relative to this Household's plans. External Coverage has no benefit categories, limits, or utilization tracking — it exists only for COB ordering. See GLO-035. | Should | User can add and configure External Coverage for a Person. Routing Engine uses it to determine submission order and recommends "process externally first" when applicable. |
