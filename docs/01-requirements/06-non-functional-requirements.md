...
| ID | Requirement | Priority | Acceptance Criteria |
|----|-------------|----------|---------------------|
| NFR-040 | *(Phase 4)* Coordinate must authenticate users before granting access to any Household data. The authentication mechanism (password + MFA, passkeys, OAuth with an identity provider, device-level auth) is deferred to the solution phase and depends on the architectural model. Not satisfiable in MVP — local-first PWA has no backend; data is accessible to whoever has device access (ADR-010). | Must (Phase 4) | No Household data is accessible without a successful authentication step. |
...
| NFR-044 | *(Phase 4)* The four delegated authorization principles from the architectural consideration below (explicit consent, scoped delegation, revocability, auditability) are security requirements, not advisory. They apply regardless of implementation mechanism. Not satisfiable in MVP — no enforcement point in a client-side-only app (ADR-010). | Must (Phase 4) | Delegation grants and revocations are logged. Access cannot be granted implicitly. Delegated access cannot exceed the scope of the granting Household and role. |
| NFR-045 | **Household data isolation**: All domain data — plan configuration, coverage limits, utilization tracking, claim status, and data retrieved via browser extension (FR-091, FR-092) — is scoped to the Household that owns the relevant Insurance Plan. Plan data is never accessible from another Household, even if the same Person belongs to both. Cross-household information flows occur only through user-initiated document sharing (EOBs, receipts). See GLO-035, FR-046. | Must | Data from one Household's plans is never visible or queryable when operating in another Household context. Browser-extension-scraped data is written only to the Household that owns the plan. |

### Architectural consideration: delegated authorization *(Phase 4)*
...
**Household as ABAC boundary** *(Phase 4)*: The Household is the attribute-based access control (ABAC) boundary for all domain data, not just a delegation scope. A System User's access to data is determined by (Household membership) × (User Role). No implicit cross-household access exists. This aligns with NFR-045 (data isolation): plan data, utilization, and insurer-retrieved data never cross Household boundaries. In MVP, the Operator has full access; Household is a data scoping boundary for UI navigation (ADR-010).

At the requirements level (Phase 4), the key principles are:
