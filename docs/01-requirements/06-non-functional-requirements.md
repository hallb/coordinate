...
**Household as ABAC boundary**: The Household is the attribute-based access control (ABAC) boundary for all domain data, not just a delegation scope. A System User's access to data is determined by (Household membership) × (User Role). No implicit cross-household access exists. This aligns with NFR-045 (data isolation): plan data, utilization, and insurer-retrieved data never cross Household boundaries.
