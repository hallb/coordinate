...
|----|------|---------------|------------|
| GLO-001 | Household | | The top-level container for a family unit using Coordinate. A Household has one or more Persons (via Household Membership), one or more Insurance Plans, and (Phase 4) one or more System Users. In MVP, the Operator manages all Households on the device. |
...
| GLO-033 | Household Membership | | The association between a Person and a Household. A Person may hold memberships in more than one Household (e.g., an adult dependent who also has their own household). The Operator selects which Household context to operate in (UI navigation). Plan Memberships, Expenses, and Submissions are scoped to a Household context. Phase 4: System User selects context after login; see GLO-007 for role scoping. |
...
| GLO-008 | Insurance Manager | Primary User, Family Insurance Manager | *(Phase 4)* A User Role (per Household Membership) with full access: configure the Household, plans, and Coordination of Benefits relationships; submit and track claims for any Person in the Household. In MVP, the Operator has equivalent access without a role assignment (ADR-010). |
