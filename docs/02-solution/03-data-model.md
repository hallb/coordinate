...
**Insurer is a value object** (`name` + `portalUrl`) on Coverage. The operator interacts with an insurer's portal to batch-submit claims across all coverage types from that insurer. Duplicating the Insurer value across coverages from the same insurer is acceptable at MVP data scale; the UI groups coverages by `insurer.name` for portal navigation.
